...
export async function handleFormSubmission(
  ...
  console.log("Form Load Time", Date.now(), Number(formLoadTime), Date.now() - Number(formLoadTime), Date.now() - Number(formLoadTime) < 1000);

  if (Date.now() - Number(formLoadTime) < 1000) { 
    return { valid: false, disabled: true, message: "Form submitted too quickly." };
  }
  ...
  if (honeypot) {
    return { valid: false, disabled: true, message: "Invalid submission detected." };
  }
